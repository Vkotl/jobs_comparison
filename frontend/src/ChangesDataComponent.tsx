...
export const ChangesDataComponent: React.FC = () => {
  const [changes, setChanges] = useState<ChangesJsonData | null>(null),
    chosen: HTMLCollectionOf<Element> =
      document.getElementsByClassName("chosen"),
    is_dates: boolean =
      document.getElementsByClassName("change-dates").length > 0;
        ...
        <Container id="changes-container">
          <Row className="text-center">
            <Col xs={12} className="position-relative">
              ...
              <h2>
                changes between {changes.previous_date.replace(/-/g, ".")} and{" "}
                {changes.new_date.replace(/-/g, ".")}
              </h2>
            </Col>
          </Row>
